...
% Select the doctor with the earliest availability for a given surgery
select_doctor_early_availability(OpCode, Day, LDoctors, BestDoctor) :-
    findall((Doctor, EarliestStart),
        (member(Doctor, LDoctors),
         availability(Doctor, Day, Availability),
         surgery_id(OpCode, OpType),
         surgery(OpType, _, TSurgery, _),
         find_earliest_start(TSurgery, Availability, EarliestStart)
        ),
        DoctorTimes),
    sort(2, @=<, DoctorTimes, [(BestDoctor, _)|_]). % Sort by EarliestStart and pick the best one.

% Find the earliest start time in a list of availability intervals
find_earliest_start(_, [], 1440). % No availability means the latest possible time (end of day)
find_earliest_start(TSurgery, [(Start, End)|_], Start) :-
    End - Start + 1 >= TSurgery, !. % First interval with sufficient time
find_earliest_start(TSurgery, [_|Rest], Earliest) :-
    find_earliest_start(TSurgery, Rest, Earliest).
...
obtain_heuristic_sol(Room, Day, AgOpRoomBetter, LAgDoctorsBetter, TFinOp) :-
    ...
    get_time(Tf),
    T is Tf-Ti,
    write('Solution based on Early Availability Heuristic:'), nl,
